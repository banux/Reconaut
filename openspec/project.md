...
- **Workers de scan spécialisés** : un binaire Go par `scan_kind` (`scanner-tcp_probe`, `scanner-tls_capture`, `scanner-http_banner`, `scanner-subdomain_enum`, `scanner-service_fingerprint`, `scanner-dns_records`), chacun consommant sa propre queue GoodJob `scan:<kind>`. La spécialisation réduit la surface d'attaque par binaire et permet de scaler chaque type indépendamment. `scanner-dns_records` (cf. change `add-dns-records-scanner`) résout les enregistrements DNS publics (A, AAAA, MX, NS, TXT, CAA, SOA, CNAME) d'un domaine couvert par le scope — pas d'AXFR. `scanner-service_fingerprint` couvre désormais **SSH, RDP et MQTT** : (a) cf. change `add-ssh-probe` — SSH banner + host-key SHA-256 sur TCP/22 ; (b) cf. change `add-rdp-probe` — RDP X.224 Negotiation + capture cert TLS opt-in sur TCP/3389 ; (c) cf. change `add-mqtt-probe` — MQTT CONNECT/CONNACK fingerprint sur TCP/1883 et /8883 (capture cert TLS opt-in sur 8883). Aucun des trois ne tente d'authentification (linters `check_ssh_probe_no_auth.sh` / `check_rdp_probe_no_auth.sh` / `check_mqtt_probe_no_auth.sh`) ; le sondeur RDP n'envoie en plus jamais de message MCS Connect-Initial ; le sondeur MQTT n'envoie jamais de PUBLISH / SUBSCRIBE / Will. `scanner-http_banner` (cf. change `add-http-probe`) couvre **HTTP et HTTPS** : status, headers, Server, extrait HTML 32 KiB, cert TLS feuille + SHA-256 + SANs + NotAfter, ALPN — `GET` / `HEAD` uniquement, pas de redirect suivi, pas d'auth, pas de payload weaponisé (linter `check_http_probe_no_offensive.sh`).
