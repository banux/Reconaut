...
- **Workers de scan spécialisés** : un binaire Go par `scan_kind` (`scanner-tcp_probe`, `scanner-tls_capture`, `scanner-http_banner`, `scanner-subdomain_enum`, `scanner-service_fingerprint`, `scanner-dns_records`), chacun consommant sa propre queue GoodJob `scan:<kind>`. La spécialisation réduit la surface d'attaque par binaire et permet de scaler chaque type indépendamment. `scanner-dns_records` (cf. change `add-dns-records-scanner`) résout les enregistrements DNS publics (A, AAAA, MX, NS, TXT, CAA, SOA, CNAME) d'un domaine couvert par le scope — pas d'AXFR. `scanner-service_fingerprint` (cf. change `add-ssh-probe`) couvre désormais le sondeur **SSH** (banner + host-key SHA-256) sur TCP/22 — premier sondeur applicatif livré, sans aucune tentative d'authentification. `scanner-http_banner` (cf. change `add-http-probe`) couvre **HTTP et HTTPS** : status, headers, Server, extrait HTML 32 KiB, cert TLS feuille + SHA-256 + SANs + NotAfter, ALPN — `GET` / `HEAD` uniquement, pas de redirect suivi, pas d'auth, pas de payload weaponisé (linter `check_http_probe_no_offensive.sh`).
