...
- **Workers de scan spécialisés** : un binaire Go par `scan_kind` (`scanner-tcp_probe`, `scanner-tls_capture`, `scanner-http_banner`, `scanner-subdomain_enum`, `scanner-service_fingerprint`, `scanner-dns_records`), chacun consommant sa propre queue GoodJob `scan:<kind>`. La spécialisation réduit la surface d'attaque par binaire et permet de scaler chaque type indépendamment. `scanner-dns_records` (cf. change `add-dns-records-scanner`) résout les enregistrements DNS publics (A, AAAA, MX, NS, TXT, CAA, SOA, CNAME) d'un domaine couvert par le scope — pas d'AXFR. `scanner-service_fingerprint` couvre désormais **SSH, RDP, MQTT, CoAP et Modbus** : (a) cf. change `add-ssh-probe` — SSH banner + host-key SHA-256 sur TCP/22 ; (b) cf. change `add-rdp-probe` — RDP X.224 Negotiation + capture cert TLS opt-in sur TCP/3389 ; (c) cf. change `add-mqtt-probe` — MQTT CONNECT/CONNACK fingerprint sur TCP/1883 et /8883 (capture cert TLS opt-in sur 8883) ; (d) cf. change `add-coap-probe` — CoAP GET `/.well-known/core` (RFC 6690) sur UDP/5683 ; (e) cf. change `add-worker-modbus` — Modbus TCP Read Device Identification (0x2B/0x0E) + fallback Read Holding Registers (0x03) sur TCP/502 pour capturer vendor/product/revision. Aucun des cinq ne tente d'authentification ni de mutation (linters `check_ssh_probe_no_auth.sh` / `check_rdp_probe_no_auth.sh` / `check_mqtt_probe_no_auth.sh` / `check_coap_probe_no_offensive.sh` / `check_modbus_probe_no_write.sh`) ; le sondeur RDP n'envoie jamais de MCS Connect-Initial ; le sondeur MQTT n'envoie jamais de PUBLISH/SUBSCRIBE/Will ; le sondeur CoAP n'envoie jamais de PUT/POST/DELETE/Observe ni de multicast ; le sondeur Modbus n'envoie jamais de write (0x05/06/0F/10/17) ni Diagnostics (0x08) et est limité à ≤ 2 paquets par sonde. **Avec HTTP/HTTPS couvert par `scanner-http_banner`, la liste §2.5 d'init-reconaut-platform est complète (6/6 sondeurs).** `scanner-http_banner` (cf. change `add-http-probe`) couvre **HTTP et HTTPS** : status, headers, Server, extrait HTML 32 KiB, cert TLS feuille + SHA-256 + SANs + NotAfter, ALPN — `GET` / `HEAD` uniquement, pas de redirect suivi, pas d'auth, pas de payload weaponisé (linter `check_http_probe_no_offensive.sh`).
