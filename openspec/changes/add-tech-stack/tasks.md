...
- [x] **3.2 `JobBus` côté Rails et consommateur côté Go**
  ...
  - **Statut** : livré côté Rails (`apps/api/app/jobs/scan_job.rb` + `apps/api/app/lib/reconaut/good_job_bus.rb`, contrat identique à `InMemoryJobBus`). Livré côté Go (`apps/scanner/internal/goodjob/` : `Job`/`Store` + `SQLStore` Postgres + `InMemoryStore` pour les tests + `Loop` avec recovery). 7 specs Rails + 9 tests Go couvrent : enqueue avec payload identique, `scan_id` correlable, parsing du `serialized_params` ActiveJob, FOR UPDATE SKIP LOCKED simulé sur store en mémoire, panic dans handler → fail + continue, erreur retournée → fail + reason loguée, ctx cancel → exit propre. La SQL réelle (sql.go) tourne contre Postgres dès qu'un driver est wired ; testcontainers-go est différé tant qu'aucun bug d'intégration n'est suspecté.
