...
2. **Ajout d'un binaire Go `reconautctl`** sous `apps/tui/` : TUI bubbletea (Charm) + sous-commandes one-shot. Le binaire parle au backend Rails via HTTPS+SSE (clé API personnelle stockée localement après login interactif). **Mise à jour 2026-05-08** (cf. change `mcp-as-primary-entrypoint`) : la couche réseau du binaire n'est PAS un client REST maison `internal/api/`, mais un **client MCP HTTP+SSE** sous `apps/tui/internal/mcp/`. Toute opération métier (scope, scan, hosts, agent, doctor) est une invocation d'outil MCP ; seuls `login`/génération de clé passent par REST (`/auth/sessions`, `/auth/api_keys`).
