...
- [x] **1.2 Retirer les règles de stack-linter relatives au web**
...
- [x] **1.3 Confirmer l'absence d'asset pipeline web côté Rails**
...
- [x] **2.1 Layout `apps/tui/` avec bubbletea**
...
- [x] **2.2 Sous-commande `reconautctl login`**
...
- [x] **2.3 Sous-commandes `reconautctl scope list|add|revoke`**
...
- [x] **2.4 Sous-commande `reconautctl scan request|status|list`**
...
- [x] **2.5 Sous-commande TUI `reconautctl agent`**
...
- [x] **2.6 Sous-commande `reconautctl doctor`**
...
- [x] **3.1 Refactor `apps/scanner/` en `apps/scanner-<kind>/`**
...
- [x] **3.2 Rails enqueue avec `queue_name` dérivé de `scan_kind`**
...
- [x] **3.3 Linter qui vérifie la spécialisation des binaires**
...
- [x] **3.4 Mise à jour de la spec `architecture` des binaires de release**
...
- [x] **4.2 Mettre à jour `add-tech-stack` (Frontend Framework + Scan Workers Runtime)**
...
- [x] **4.3 Mettre à jour `init-reconaut-platform/specs/platform/spec.md`**
...
- [x] **4.4 Mettre à jour `README.md` racine et `docker-compose.yml`**
...
- [x] **5.1 Tests automatisés des nouveaux requirements**
...
- [x] **5.2 Linter de stack mis à jour bloque la résurgence du web**
...
- [x] **5.3 Linter de spécialisation des workers actif en CI**
  - `scripts/check_scanner_specialization.sh` tourne sur chaque PR et bloque tout import croisé entre packages de protocole.

- [x] **5.4 Le binaire `reconautctl` est utilisable en environnement minimal**
...
- [x] **5.5 Aucun composant web ne ressuscite par mégarde**
