...
- [ ] **1.2 `ApiKey` indépendant de `user_id`**
...
- [ ] **1.3 Constante `VALID_ROLES` retirée**
...
- [ ] **2.2 Use cases sans `caller_role:`**
...
- [ ] **2.3 Constantes `READ_ROLES` / `WRITE_ROLES` / `AUTHORIZED_ROLES` retirées**
...
- [ ] **2.4 `MCP::ToolsController::SCOPES_BY_ROLE` retirée**
...
- [ ] **3.1 Retirer toute référence OIDC**
...
- [ ] **4.1 Renommer `bootstrap_owner` → `set_password`**
...
- [ ] **4.2 Endpoint `POST /auth/sessions` accepte juste `password`**
...
- [ ] **5.1 Retirer `list_users` du registry**
...
- [ ] **5.2 `list_api_keys` perd le paramètre `user_id`**
...
- [ ] **5.3 Anticiper `grant_role`/`revoke_role` non implémentés**
...
- [ ] **6.1 Mettre à jour `init-reconaut-platform/specs/platform/spec.md`**
...
- [ ] **6.2 Mettre à jour `mcp-as-primary-entrypoint/specs/mcp-server/spec.md`**
...
- [ ] **6.3 Mettre à jour `replace-web-with-tui/specs/platform/spec.md`**
...
- [ ] **6.4 Mettre à jour `openspec/project.md`**
...
- [ ] **7.1 Tests automatisés alignés sur le mono-user**
...
- [ ] **7.4 La routine `doctor` reflète l'absence de RBAC multi-rôle**
