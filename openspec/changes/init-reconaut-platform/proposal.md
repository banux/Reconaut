...
5. **~~gdpr-compliance~~** — capacité retirée par le change `drop-gdpr-framing`. Les outils opérationnels qui restent (audit append-only, effacement par cible, étiquette de résidence) sont décrits dans les specs `platform` et `scanning` modifiées.
...
- **L'opérateur reste responsable de la légalité et de l'éthique de ses scans.** Reconaut applique le scope déclaré et fournit des outils opérationnels (audit append-only, effacement par cible, étiquette de résidence) — pas un framework de conformité. Cf. change `drop-gdpr-framing`.
...
3. **Outils opérationnels, pas framework de conformité.** Reconaut fournit l'audit append-only, l'effacement par cible et l'étiquette de résidence comme **outils opérationnels** (forensique, hygiène, souveraineté). Aucune feature applicative n'est cadrée par RGPD — le projet ne stocke pas de PII et ne se prononce pas sur la conformité de l'opérateur. Cf. change `drop-gdpr-framing`.
