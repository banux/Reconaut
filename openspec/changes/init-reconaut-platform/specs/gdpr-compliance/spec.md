...
La plateforme DOIT fournir à l'opérateur un workflow d'effacement par identifiant (IP, domaine, ou `host_id`). L'effacement DOIT être transactionnel : couvre simultanément la base OLTP, l'index vectoriel, le graphe (si la capacité `graph-retrieval` est active) et le tier froid (Postgres compressé ou filesystem selon la configuration). Une tombstone hashée DOIT être écrite dans le journal d'audit. Le workflow ne tente pas de valider le « contrôle de la cible » par la personne concernée — c'est à l'opérateur, en tant que controller, de définir et d'exécuter sa procédure de vérification interne. La plateforme se borne à exécuter l'effacement de manière atomique et auditable une fois que l'opérateur l'a déclenché.
...
Les sous-traitants externes (fournisseur d'embeddings, fournisseur d'auth, etc.) DOIVENT être encadrés par un Data Processing Agreement conforme à l'Art. 28 RGPD **entre l'opérateur et le fournisseur** — la plateforme ne contracte avec aucun fournisseur en lieu et place de l'opérateur. Toute intégration livrée (par ex. embedder Mistral) DOIT être désactivable et **désactivée par défaut**. Quand activée, la plateforme DOIT logger l'identité du fournisseur et l'endpoint utilisé, à des fins d'audit et de transparence pour l'opérateur.
