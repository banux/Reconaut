...
Statut : **acté** par le change [`mcp-as-primary-entrypoint`](https://github.com/banux/Reconaut/blob/main/openspec/changes/mcp-as-primary-entrypoint/proposal.md).
...
1. **Une API REST** historique (`POST /agent/chat`, `resources :scopes`, …) consommée par la SPA Vue puis, après [`replace-web-with-tui`](https://github.com/banux/Reconaut/blob/main/openspec/changes/replace-web-with-tui/), par le binaire `reconautctl`.
...
Le change [`mcp-as-primary-entrypoint`](https://github.com/banux/Reconaut/blob/main/openspec/changes/mcp-as-primary-entrypoint/) acte qu'on **collapse les deux surfaces sur MCP** :

- Une seule clé API personnelle par opérateur, valable pour TUI ET agents IA (cf. spec [`platform`](https://github.com/banux/Reconaut/blob/main/openspec/changes/mcp-as-primary-entrypoint/specs/platform/spec.md)).
...
Le linter [`scripts/check_rest_allowlist.sh`](https://github.com/banux/Reconaut/blob/main/scripts/check_rest_allowlist.sh) tourne en CI et rejette toute nouvelle route hors de l'allowlist.
...
2. **Définir le schéma de paramètres typé**. Le registre MCP utilise un mini-langage (`type: :string|:integer|:enum|:hash`, `min_length`, `max_length`, `min`, `max`, `required`, `default`). Cf. [`apps/api/app/lib/mcp/tool_registry.rb`](https://github.com/banux/Reconaut/blob/main/apps/api/app/lib/mcp/tool_registry.rb).
3. **Choisir le scope RBAC** dans la matrice (cf. spec [`mcp-server`](https://github.com/banux/Reconaut/blob/main/openspec/changes/single-user-only/specs/mcp-server/spec.md) après `single-user-only`). Exemples : `read:hosts`, `write:scans`, `agent:chat`, `read:health`, `write:api_keys`.
4. **Enregistrer l'outil** dans `Mcp::CoreTools.register_all!` ([apps/api/app/lib/mcp/core_tools.rb](https://github.com/banux/Reconaut/blob/main/apps/api/app/lib/mcp/core_tools.rb)). Le handler reçoit `params:` et `caller_id:`, et doit retourner un Hash sérialisable JSON.
5. **Ajouter à `OPERATOR_SCOPES`** ([apps/api/app/controllers/mcp/tools_controller.rb](https://github.com/banux/Reconaut/blob/main/apps/api/app/controllers/mcp/tools_controller.rb)) si le scope est nouveau ; sinon le test `operator_scopes_spec.rb` plante au boot.
...
- Après : tool MCP `agent_chat` enregistré dans [`Mcp::CoreTools`](https://github.com/banux/Reconaut/blob/main/apps/api/app/lib/mcp/core_tools.rb) ; le handler appelle `retriever.call(prompt)` et renvoie le même Hash.
- Streaming : le controller [`Mcp::ToolsController`](https://github.com/banux/Reconaut/blob/main/apps/api/app/controllers/mcp/tools_controller.rb) détecte `Accept: text/event-stream` et bascule sur [`Mcp::AgentChatStreamer`](https://github.com/banux/Reconaut/blob/main/apps/api/app/lib/mcp/agent_chat_streamer.rb) qui découpe la réponse en chunks `tool_result` partiels (cf. §1.2). Le change [`add-agent-chat-streaming`](https://github.com/banux/Reconaut/blob/main/openspec/changes/add-agent-chat-streaming/) ajoute heartbeat keep-alive (`event: ping`), cancellation propagation (audit `client_gone`), et émission progressive optionnelle via `each_chunk`. Format détaillé et bonnes pratiques SDK consommateurs : [`docs/operating/agent-chat-streaming.md`](../operating/agent-chat-streaming.md).
...
- La TUI `reconautctl` utilise exclusivement MCP (sauf auth bootstrap) — vérifié par [`scripts/check_tui_mcp_only.sh`](https://github.com/banux/Reconaut/blob/main/scripts/check_tui_mcp_only.sh).
...
3. Met à jour le linter d'allowlist en retirant la zone de transition (`TRANSITION_PATTERNS` dans [`scripts/check_rest_allowlist.sh`](https://github.com/banux/Reconaut/blob/main/scripts/check_rest_allowlist.sh)).

## Liens

- [Spec mcp-server (single-user-only)](https://github.com/banux/Reconaut/blob/main/openspec/changes/single-user-only/specs/mcp-server/spec.md) — matrice de scopes définitive en mono-user.
- [Spec mcp-server (mcp-as-primary-entrypoint)](https://github.com/banux/Reconaut/blob/main/openspec/changes/mcp-as-primary-entrypoint/specs/mcp-server/spec.md) — Requirement *REST API Reduced to Bootstrap, Health and MCP Transport*.
- [Spec architecture](https://github.com/banux/Reconaut/blob/main/openspec/changes/mcp-as-primary-entrypoint/specs/architecture/spec.md) — Requirement *MCP HTTP+SSE as Primary Entrypoint*.
